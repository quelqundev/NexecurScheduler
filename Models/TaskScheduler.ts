...
import { SMSAlert, SMSAlertStatus } from "./SMSAlert";
...
export class TaskScheduler {
    ...
    constructor(scheduleConfig: Configuration) {
         ...
         */
        this.activationSchedule = schedule.scheduleJob({ hour: scheduleConfig.activationHour }, this.activationFunction);
        ...
        let postcheckCallback = (status: AlarmStatus) => {
            switch (status) {
                ...
                case AlarmStatus.Disabled:
                    ...
                    SMSAlert.sendSMSAlert(`ACTIVATION ECHOUEE a ${this.scheduleConfig.activationHour}h. Alarme encore DESACTIVEE.`);
                    break;
                default:
        ...
        let postcheckCallback = (status: AlarmStatus) => {
            switch (status) {
                ...
                case AlarmStatus.Enabled:
                    ...
                    SMSAlert
                        .sendSMSAlert(`URGENT ! DESACTIVATION ECHOUEE a ${this.scheduleConfig.desactivationHour}h. Alarme encore ACTIVEE. Veuillez utiliser l'application Mon Nexecur pour desarmer.`)
                        .catch((status: SMSAlertStatus) => {
                            Log.info("Erreur lors de l envoi du SMS : " + status);
                            //we failed at disabling alarm, let's retry
                            this.desactivationFunction();
                        });
                    break;
                default:
                    //error : status is not an AlarmStatus
